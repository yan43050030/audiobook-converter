...
class AudiobookConverterApp:
    ...
    def _init_sash_position(self, paned: "ttk.PanedWindow"):
        """让右侧面板（侧栏+设置）初始约占 50% 宽度"""
        try:
            paned.update_idletasks()
            total = paned.winfo_width()
            if total <= 0:
                return
            sash_x = int(total * 0.55)
    ...
    def _build_ui(self):
        ...
        ttk.Label(search_frame, text="🔍 过滤", font=("Helvetica", 10), foreground="gray").pack(side=tk.RIGHT, padx=(4, 0))
        ...
        # ===== 右侧：侧栏导航 + 设置面板 =====
        right_outer = ttk.Frame(body, padding=2)
        body.add(right_outer, weight=2)
        self.root.after(50, lambda: self._init_sash_position(body))

        # 侧栏
        sidebar = self._build_sidebar(right_outer)
        sidebar.pack(side=tk.LEFT, fill=tk.Y)

        # 面板容器
        self._panel_container = ttk.Frame(right_outer)
        self._panel_container.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # 创建所有面板
        self._panels = {}
        for pid, _label in self.SIDEBAR_ITEMS:
            method = getattr(self, f"_build_panel_{pid}", None)
            if method:
                frame = method(self._panel_container)
                self._panels[pid] = frame

        # 默认显示引擎面板
        self._show_panel("engine")

        # 底部进度和状态（放在 main 底部，notebook 下方）
        ...
        # 快速操作栏 + 状态栏
        bar_frame = ttk.Frame(main)
        bar_frame.pack(fill=tk.X, side=tk.BOTTOM)
        self._build_statusbar(bar_frame)
        self._build_quickbar(bar_frame)
    ...
    def _add_model_folder(self):
        """允许用户选择包含语音模型的文件夹，加入搜索路径"""
        path = filedialog.askdirectory(title="选择包含语音模型（.onnx / .pt）的文件夹")
        if not path:
            return
        try:
            from tts_engine import add_model_search_path
            added = add_model_search_path(path)
            if added:
                self._refresh_deps()
                messagebox.showinfo("已添加", f"已将模型文件夹加入搜索路径：\n{path}")
            else:
                messagebox.showinfo("提示", f"该文件夹已在搜索路径中：\n{path}")
        except Exception as e:
            logger.error(f"添加模型文件夹失败: {e}")
            messagebox.showerror("错误", str(e))

    def _add_binary_folder(self):
        """允许用户选择包含可执行文件的文件夹，加入搜索路径"""
        path = filedialog.askdirectory(title="选择包含 ffmpeg / piper 等可执行文件的文件夹")
        if not path:
            return
        try:
            from tts_engine import add_model_search_path
            added = add_model_search_path(path)
            self._invalidate_portable_cache()
            if added:
                self._refresh_deps()
                messagebox.showinfo("已添加", f"已将程序文件夹加入搜索路径：\n{path}")
            else:
                messagebox.showinfo("提示", f"该文件夹已在搜索路径中：\n{path}")
        except Exception as e:
            logger.error(f"添加程序文件夹失败: {e}")
            messagebox.showerror("错误", str(e))

    def _invalidate_portable_cache(self):
        """使便携目录扫描缓存失效，强制重新搜索"""
        try:
            from tts_engine import _invalidate_scan_cache
    ...
    def _refresh_deps(self):
        ...
        if info["missing"]:
            ...
            if any("ffmpeg" in m for m in info["missing"]):
                ...
                lines.append("   或 macOS: brew install ffmpeg")
            if any("Piper" in m for m in info["missing"]):
                lines.append("   Piper: https://github.com/rhasspy/piper/releases")
                lines.append("   模型: https://huggingface.co/rhasspy/piper-voices")
        ...
        try:
            ...
            lines.append("")
            lines.append(f"{status_icon} 当前引擎: {msg}")
        except Exception:
            pass
    ...
    def _rebuild_engine_buttons(self):
        """从注册表动态创建所有引擎单选按钮"""
        if not hasattr(self, "_engine_buttons_frame"):
            return
        ...
        BUILTIN_NAMES = {
            ...
            "local": "本地（离线）",
            "piper": "Piper（离线高质量）",
            "cosyvoice": "CosyVoice（离线神经）",
        }
        for eid, info in engines.items():
            if info["type"] == "builtin":
                name = BUILTIN_NAMES.get(eid, info.get("name", eid))
                ready, _msg = check_engine_ready(eid)
                if not ready:
                    name += "（需安装）"
                rb = ttk.Radiobutton(
                    self._engine_buttons_frame, text=name,
                    variable=self.engine_var, value=eid,
                    command=self._on_engine_change,
                )
            else:
                rb = ttk.Radiobutton(
                    self._engine_buttons_frame,
                    text=f"⚡ {info['name']}（外挂）",
                    variable=self.engine_var, value=eid,
                    command=self._on_engine_change,
                )
            rb.pack(anchor=tk.W)
            self._engine_radio_buttons.append(rb)
    ...
    def _set_engine_controls_state(self, state: str):
        ...
        try:
            ...
            pass
        try:
            self.btn_convert_qb.configure(state=state)
        except Exception:
            pass
    ...
    def _update_quickbar(self):
        ...
        try:
            engine = self.engine_var.get()
            ready, _msg = check_engine_ready(engine)
            dot = "●" if ready else "○"
            names = {"edge": "Edge", "local": "本地", "piper": "Piper",
                     "cosyvoice": "CosyVoice"}
            self._qb_engine_label.config(text=f"{dot} {names.get(engine, engine)}")
        except Exception:
            pass
        try:
            voice = self.voice_var.get()
            self._qb_voice_label.config(text=f"▸ {voice}" if voice else "▸ 未选择语音")
        except Exception:
            pass
    ...
    def _update_statusbar(self):
        ...
        try:
            engine = self.engine_var.get()
            ready, msg = check_engine_ready(engine)
            status_text = "● 就绪" if ready else "○ 引擎不可用"
            self._sb_status.config(text=status_text,
                                   foreground="green" if ready else "red")
            self._sb_engine.config(text=f"引擎: {msg[:30]}")
            voices = get_voice_list(engine)
            self._sb_voices.config(text=f"{len(voices)} 语音")
        except Exception:
            pass
    ...
    def _on_engine_change(self):
        ...
        for btn in [getattr(self, "btn_convert", None), getattr(self, "btn_convert_qb", None)]:
            if btn:
    ...
    def _toggle_theme(self):
        ...
        try:
            self._theme_status.config(text=f"当前: {new} 模式")
        except Exception:
            pass
        try:
            from tts_engine import _load_config as _lc, _save_config as _sc
    ...
    def _update_rate_label(self, value):
        ...
        if val == 0:
            ...
            label = f"慢 {val}%"
        self.rate_label.config(text=label)
        try:
            self._qb_rate_label.config(text=label)
        except Exception:
            pass
    ...
    def _run_convert(self, output_dir: str, file_prefix: str, selected_indices: list, resume: bool):
        ...
        self.btn_convert_qb.config(state="disabled")
        self.btn_pause_qb.config(state="normal")
        ...
        def run():
            try:
                ...
                self.root.after(0, lambda: self.btn_convert_qb.config(state="normal"))
                self.root.after(0, lambda: self.btn_pause_qb.config(state="disabled"))
    ...
    def _build_sidebar(self, parent):
        ...
        # Header
        hdr = ttk.Frame(bar)
        hdr.pack(fill=tk.X, padx=6, pady=(8, 10))
        ttk.Label(hdr, text="导航", font=("Helvetica", 10, "bold")).pack()
        # Nav buttons
        self._sidebar_btns = {}
        for pid, label in self.SIDEBAR_ITEMS:
            btn = ttk.Button(bar, text=label, command=lambda p=pid: self._show_panel(p))
    ...
    def _build_panel_engine(self, parent):
        """引擎与语音面板"""
        f = ttk.Frame(parent)
        pad = 4
        # 引擎选择卡片
        engine_card = ttk.LabelFrame(f, text="选择引擎", padding=pad)
        engine_card.pack(fill=tk.X, pady=(0, pad))
        self.engine_var = tk.StringVar(value="edge")
        self._engine_radio_buttons = []
        self._engine_buttons_frame = ttk.Frame(engine_card)
        self._engine_buttons_frame.pack(fill=tk.X)
        self._rebuild_engine_buttons()
        # 添加外挂引擎
        ext_row = ttk.Frame(engine_card)
        ext_row.pack(fill=tk.X, pady=(4, 0))
        self._ext_btn = ttk.Button(ext_row, text="+ 添加外挂引擎",
                                   command=self._open_external_dialog)
        self._ext_btn.pack(side=tk.LEFT)
        # 语音参数
        voice_card = ttk.LabelFrame(f, text="语音参数", padding=pad)
        voice_card.pack(fill=tk.X, pady=(0, pad))
        vr = ttk.Frame(voice_card)
        vr.pack(fill=tk.X)
        ttk.Label(vr, text="语音:").pack(side=tk.LEFT)
        self.voice_var = tk.StringVar()
        self.voice_combo = ttk.Combobox(vr, textvariable=self.voice_var,
                                        state="readonly", width=22)
        self.voice_combo.pack(side=tk.LEFT, padx=4, fill=tk.X, expand=True)
        self._preview_btn = ttk.Button(vr, text="试听", width=5,
                                       command=self._preview_voice_sample)
        self._preview_btn.pack(side=tk.RIGHT, padx=(0, 2))
        self._refresh_btn = ttk.Button(vr, text="刷新", width=5,
                                       command=self._refresh_voices)
        self._refresh_btn.pack(side=tk.RIGHT)
        self._on_engine_change()
        return f

    def _build_panel_speed(self, parent):
        """语速与输出面板"""
        f = ttk.Frame(parent)
        pad = 4
        # 语速
        rate_card = ttk.LabelFrame(f, text="语速控制", padding=pad)
        rate_card.pack(fill=tk.X, pady=(0, pad))
        self.rate_var = tk.IntVar(value=0)
        self.rate_label = ttk.Label(rate_card, text="正常")
        self.rate_label.pack(anchor=tk.E)
        ttk.Scale(rate_card, from_=-50, to=50, variable=self.rate_var,
                  command=self._update_rate_label).pack(fill=tk.X)
        # 输出模式
        out_card = ttk.LabelFrame(f, text="输出模式", padding=pad)
        out_card.pack(fill=tk.X, pady=(0, pad))
        self.mode_var = tk.StringVar(value="chapter")
        for label, val in [("按章节拆分", "chapter"), ("按时间拆分", "time"),
                           ("合并为一个文件", "single")]:
            ttk.Radiobutton(out_card, text=label, variable=self.mode_var,
                            value=val, command=self._on_mode_change).pack(anchor=tk.W)
        self.time_frame = ttk.Frame(out_card)
        ttk.Label(self.time_frame, text="每段:").pack(side=tk.LEFT)
        self.time_var = tk.IntVar(value=30)
        sp = ttk.Spinbox(self.time_frame, from_=5, to=180, textvariable=self.time_var,
                         width=5, increment=5)
        sp.pack(side=tk.LEFT, padx=pad)
        sp.configure(command=self._update_split_estimate)
        self.time_var.trace_add("write", lambda *a: self._update_split_estimate())
        ttk.Label(self.time_frame, text="分钟").pack(side=tk.LEFT)
        self.split_estimate_label = ttk.Label(out_card, text="", foreground="gray",
                                              wraplength=280)
        self.split_estimate_label.pack(fill=tk.X, pady=(2, 0))
        # 响度归一化
        self.normalize_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(out_card, text="响度归一化（统一各文件音量，需 ffmpeg）",
                        variable=self.normalize_var).pack(fill=tk.X, anchor=tk.W, pady=(4, 0))
        self._on_mode_change()
        return f

    def _build_panel_dialogue(self, parent):
        """对话识别面板"""
        f = ttk.Frame(parent)
        pad = 4
        card = ttk.LabelFrame(f, text="多人对话", padding=pad)
        card.pack(fill=tk.X)
        self.dialogue_enabled_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(card, text="启用对话检测（旁白/对话不同语音）",
                        variable=self.dialogue_enabled_var,
                        command=self._on_dialogue_toggle).pack(fill=tk.X, anchor=tk.W, pady=(0, pad))
        nr = ttk.Frame(card)
        nr.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(nr, text="旁白语音:").pack(side=tk.LEFT)
        self.dialogue_narration_var = tk.StringVar()
        self.dialogue_narration_combo = ttk.Combobox(nr, textvariable=self.dialogue_narration_var,
                                                     state="readonly", width=20)
        self.dialogue_narration_combo.pack(side=tk.RIGHT)
        dr = ttk.Frame(card)
        dr.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(dr, text="对话语音:").pack(side=tk.LEFT)
        self.dialogue_voice_var = tk.StringVar()
        self.dialogue_voice_combo = ttk.Combobox(dr, textvariable=self.dialogue_voice_var,
                                                 state="readonly", width=20)
        self.dialogue_voice_combo.pack(side=tk.RIGHT)
        self._on_dialogue_toggle()
        return f

    def _build_panel_storage(self, parent):
        """存储与依赖面板"""
        f = ttk.Frame(parent)
        pad = 4
        # 存储目录
        dir_card = ttk.LabelFrame(f, text="便携存储目录", padding=pad)
        dir_card.pack(fill=tk.X, pady=(0, pad))
        self.storage_var = tk.StringVar(value=get_storage_dir())
        ttk.Entry(dir_card, textvariable=self.storage_var, state="readonly").pack(fill=tk.X, pady=(0, pad))
        srow = ttk.Frame(dir_card)
        srow.pack(fill=tk.X)
        ttk.Button(srow, text="选择文件夹", command=self._choose_storage).pack(side=tk.LEFT, padx=(0, pad))
        ttk.Button(srow, text="打开", command=self._open_storage).pack(side=tk.LEFT, padx=(0, pad))
        ttk.Button(srow, text="恢复默认", command=self._reset_storage).pack(side=tk.LEFT)
        ttk.Label(dir_card, text="bin/ 存放可执行文件；piper-models/ 存放语音包。程序自动在子目录搜索。",
                  wraplength=280, foreground="gray", font=("Helvetica", 10)).pack(fill=tk.X, pady=(pad, 0))
        # 依赖检测
        deps_card = ttk.LabelFrame(f, text="依赖检测", padding=pad)
        deps_card.pack(fill=tk.X, pady=(0, pad))
        self.deps_text = tk.Text(deps_card, height=14, wrap=tk.WORD, relief=tk.FLAT,
                                 font=("Helvetica", 11), bg="#ffffff")
        self.deps_text.pack(fill=tk.BOTH, expand=True)
        self.deps_text.configure(state="disabled")
        # 操作按钮
        brow = ttk.Frame(deps_card)
        brow.pack(fill=tk.X, pady=(pad, 0))
        ttk.Button(brow, text="⚙ 重新扫描", command=self._refresh_deps).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2))
        ttk.Button(brow, text="⬇ 下载 Piper 模型", command=self._download_piper_models).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(2, 0))
        brow2 = ttk.Frame(deps_card)
        brow2.pack(fill=tk.X, pady=(2, 0))
        ttk.Button(brow2, text="⬇ 下载 CosyVoice 模型", command=self._download_cosyvoice_models).pack(fill=tk.X)
        brow3 = ttk.Frame(deps_card)
        brow3.pack(fill=tk.X, pady=(2, 0))
        ttk.Button(brow3, text="📁 添加模型文件夹", command=self._add_model_folder).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2))
        ttk.Button(brow3, text="📁 添加程序文件夹", command=self._add_binary_folder).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(2, 0))
        return f

    def _build_panel_files(self, parent):
        """文件管理面板"""
        f = ttk.Frame(parent)
        pad = 4
        card = ttk.LabelFrame(f, text="已加载文件", padding=pad)
        card.pack(fill=tk.BOTH, expand=True)
        # 按钮
        frow = ttk.Frame(card)
        frow.pack(fill=tk.X, pady=(0, pad))
        ttk.Button(frow, text="📂 添加文件（可多选）", command=self._add_files).pack(side=tk.LEFT, padx=(0, pad))
        ttk.Button(frow, text="🗑 移除选中", command=self._remove_selected_file).pack(side=tk.LEFT)
        # 文件列表
        self.file_tree = ttk.Treeview(card, columns=("name",), show="tree", height=10)
        self.file_tree.pack(fill=tk.BOTH, expand=True, pady=(0, pad))
        self.file_tree.bind("<Delete>", lambda e: self._remove_selected_file())
        self.file_count_label = ttk.Label(card, text="未加载文件", foreground="gray")
        self.file_count_label.pack(fill=tk.X)
        return f

    def _build_panel_appear(self, parent):
        """外观面板"""
        f = ttk.Frame(parent)
        pad = 4
        card = ttk.LabelFrame(f, text="主题", padding=pad)
        card.pack(fill=tk.X)
        self._theme_btn = ttk.Button(card, text="🔄 切换深色/浅色主题",
                                     command=self._toggle_theme)
        self._theme_btn.pack(fill=tk.X, pady=(0, pad))
        import sv_ttk
        current = sv_ttk.get_theme()
        self._theme_status = ttk.Label(card, text=f"当前: {current} 模式",
                                       foreground="gray")
        self._theme_status.pack()
        return f

    def _build_panel_ops(self, parent):
        """操作面板"""
        f = ttk.Frame(parent)
        pad = 4
        card = ttk.LabelFrame(f, text="工具", padding=pad)
        card.pack(fill=tk.X)
        self.btn_preview_full = ttk.Button(card, text="🔊 试听全文（可暂停）",
                                           command=self._toggle_preview_full)
        self.btn_preview_full.pack(fill=tk.X, pady=(0, pad))
        self.btn_convert = ttk.Button(card, text="▶ 生成MP3", command=self._start_convert,
                                      style="Accent.TButton")
        self.btn_convert.pack(fill=tk.X, pady=(0, pad))
        self.btn_pause = ttk.Button(card, text="⏹ 暂停", command=self._pause_convert,
                                    state="disabled")
        self.btn_pause.pack(fill=tk.X, pady=(0, pad))
        self.btn_resume = ttk.Button(card, text="▶ 继续生成", command=self._resume_convert)
        self.btn_resume.pack(fill=tk.X, pady=(0, pad))
        ttk.Button(card, text="🔀 合并MP3文件", command=self._merge_mp3).pack(fill=tk.X, pady=(0, pad))
        ttk.Button(card, text="📋 查看日志", command=self._show_log).pack(fill=tk.X)
        return f

    def _build_quickbar(self, parent):
        """底部快速操作栏"""
        bar = ttk.Frame(parent, style="QuickBar.TFrame")
        bar.pack(fill=tk.X, side=tk.BOTTOM, pady=(0, 0))
        pad = 4
        inner = ttk.Frame(bar, padding=(8, 6))
        inner.pack(fill=tk.X)
        # 引擎指示
        self._qb_engine_label = ttk.Label(inner, text="● Edge", font=("Helvetica", 11, "bold"))
        self._qb_engine_label.pack(side=tk.LEFT, padx=(0, 16))
        # 语音
        self._qb_voice_label = ttk.Label(inner, text="▸ 未选择语音", font=("Helvetica", 10))
        self._qb_voice_label.pack(side=tk.LEFT, padx=(0, 16))
        # 语速快捷
        ttk.Label(inner, text="语速:").pack(side=tk.LEFT)
        self._qb_rate_label = ttk.Label(inner, text="正常", font=("Helvetica", 10, "bold"),
                                        foreground="#2563EB", width=8, anchor=tk.W)
        self._qb_rate_label.pack(side=tk.LEFT, padx=(4, 16))
        # 操作按钮
        self.btn_convert_qb = ttk.Button(inner, text="▶ 开始转换",
                                         command=self._start_convert, style="Accent.TButton")
        self.btn_convert_qb.pack(side=tk.RIGHT, padx=(4, 0))
        self.btn_pause_qb = ttk.Button(inner, text="⏹ 停止",
                                       command=self._pause_convert, state="disabled")
        self.btn_pause_qb.pack(side=tk.RIGHT)
        return bar
    ...
    def _build_statusbar(self, parent):
        ...
        inner = ttk.Frame(bar, padding=(8, 3))
        inner.pack(fill=tk.X)
        # 状态指示
        self._sb_status = ttk.Label(inner, text="● 就绪", font=("Helvetica", 10),
                                    foreground="gray")
        self._sb_status.pack(side=tk.LEFT)
        ttk.Label(inner, text="|", foreground="#ccc").pack(side=tk.LEFT, padx=8)
        self._sb_engine = ttk.Label(inner, text="引擎: Edge（联网）", font=("Helvetica", 10),
                                    foreground="gray")
        self._sb_engine.pack(side=tk.LEFT)
        ttk.Label(inner, text="|", foreground="#ccc").pack(side=tk.LEFT, padx=8)
        self._sb_voices = ttk.Label(inner, text="6 语音", font=("Helvetica", 10),
                                    foreground="gray")
        self._sb_voices.pack(side=tk.LEFT)
        # 版本号
        ttk.Label(inner, text=f"v{VERSION}", font=("Helvetica", 10),
    ...
    def _build_asr_tab(self):
        ...
        ttk.Label(asr_top, text=model_desc, wraplength=400, foreground="gray",
                  font=("Helvetica", 10)).pack(fill=tk.X, pady=(0, pad))
