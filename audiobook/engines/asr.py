"""audiobook.engines.asr — ASR（语音转文字）引擎，基于 faster-whisper。

v6.0/A4 从顶层 `asr_engine` 迁入本包；顶层 `asr_engine` 保留为薄再导出垫片以
保持旧 import 与 PyInstaller 导入图兼容。对 `tts_engine` 的引用均为函数内惰性
导入，迁移后仍指向顶层 `tts_engine`，不受影响。
"""
