...
**v5.2.1** — 将文字转换为自然语音的有声读物制作工具，PySide6/Qt6 界面，支持多引擎 TTS、ASR 语音转文字、多人对话识别。v5.2 聚焦功能扩展：Piper 语音包在线目录（数百个多语言语音）、角色→音色映射、m4b 有声书导出（含章节）、srt 字幕同步生成、ASR 批量转录。v5.2.1 修复 Linux 本地语音检测并优化界面布局。
...
- **多人对话识别**：旁白/对话自动区分，支持角色→音色映射（从文本一键检测角色）
- **灵活拆分**：按章节拆分、按时长拆分、合并为单个文件
- **断点续传**：批量生成中断后可从中断处继续
- **丰富输出**：MP3 合并、m4b 有声书导出（内嵌章节标记）、ID3 元数据写入、srt 字幕同步生成
- **Piper 语音市场**：内置官方语音目录，界面内直接下载数百个多语言语音包（含镜像加速）
